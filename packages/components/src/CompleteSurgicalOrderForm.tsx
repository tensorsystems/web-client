...
import {
  ConfirmSurgicalOrderInput,
  MutationConfirmSurgicalOrderArgs,
  PaginationInput,
  Query,
...
const CONFIRM_SURGICAL_ORDER = gql`
  mutation ConfirmSurgicalOrder($input: ConfirmSurgicalOrderInput!) {
    confirmSurgicalOrder(input: $input) {
      surgicalOrder {
        id
      }
      surgicalProcedureId
...
interface Props {
  selectedOrder: SurgicalOrder;
  onSuccess: () => void;
  onCancel: () => void;
  onRefresh: () => void;
}

export const CompleteSurgicalOrderForm: React.FC<Props> = ({
  selectedOrder,
  onSuccess,
  onCancel,
}) => {
  ...
  const { register, handleSubmit, setValue, watch } =
    useForm<ConfirmSurgicalOrderInput>();
  const orderValues = watch();

  const [order, setOrder] = useState<SurgicalOrder>(selectedOrder);
  ...
  const [confirmSurgicalOrder] = useMutation<
    any,
    MutationConfirmSurgicalOrderArgs
  >(CONFIRM_SURGICAL_ORDER, {
    onCompleted(data) {
      onSuccess();

      const surgicalProcedureId = data.confirmSurgicalOrder.surgicalProcedureId;
      const surgicalProcedureIdx = order.surgicalProcedures.findIndex(
        (e) => e.id === surgicalProcedureId
      );

      setOrder({
        ...order,
        surgicalProcedures: [
          ...order.surgicalProcedures.slice(0, surgicalProcedureIdx),
          {
            ...order.surgicalProcedures[surgicalProcedureIdx],
            payments: order.surgicalProcedures[
              surgicalProcedureIdx
            ].payments.map((e) => ({
              ...e,
              status: "PAID",
              invoiceNo: data.confirmSurgicalOrder.invoiceNo,
            })),
          },
          ...order.surgicalProcedures.slice(surgicalProcedureIdx + 1),
        ],
      });
    },
    onError(error) {
  ...
  useEffect(() => {
    const operatingRoom = lookupQuery.data?.rooms.edges.find(
      (e) => e?.node.title === "Operating Room 1"
    );

    if (operatingRoom) {
      setValue("roomId", operatingRoom.node.id);
    }
  }, [lookupQuery.data?.rooms]);
  ...
  const handleFormSubmit = (
    values: ConfirmSurgicalOrderInput,
    surgicalProcedureId: string
  ) => {
    values.surgicalOrderId = selectedOrder.id;
    values.surgicalProcedureId = surgicalProcedureId;
  ...
  const hasPendingPayments = order.surgicalProcedures
    .map((e) => e.payments)
    .flat()
      ...
      <div className="container mx-auto">
        ...
        <div className="flex space-x-10 divide-x-2 divide-gray-300 h-full">
          <div className="flex-1">
            <p className="text-2xl text-teal-700 font-semibold tracking-wider">{`Surgical Order for ${order.firstName} ${order.lastName}`}</p>

            {order.surgicalProcedures.map((procedure, index) => {
                  ...
                  <AccordionItem
                    title={
                      <p className="text-center font-semibold text-xl text-white">
                        {procedure.surgicalProcedureType.title}
                      </p>
                    }
