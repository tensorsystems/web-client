...
export const UpdatePastSurgeryForm: React.FC<UpdatePastSurgeryProps> = ({
  values,
  onSuccess,
  onCancel,
  onSaveChange,
  ...
  const { register, handleSubmit } = useForm<PastSurgeryUpdateInput>({
    defaultValues: {
      id: values?.id,
      description: values?.description,
      surgeryDate: format(parseISO(values?.surgeryDate), "yyyy-MM-dd"),
    },
  });

  const [save, { error }] = useMutation<any, MutationUpdatePastSurgeryArgs>(
    UPDATE_PAST_SURGERY,
    {
      onCompleted(data) {
  ...
  const onSubmit = (data: PastSurgeryUpdateInput) => {
