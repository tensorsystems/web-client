...
const SystemAdminTable: React.FC = () => {
  ...
  const { data, refetch } = useQuery<Query, QuerySystemsArgs>(SYSTEMS, {
  ...
  const handleNextClick = () => {
    const totalPages = data?.systems.pageInfo.totalPages ?? 0;
          ...
          <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data?.systems.edges.map((value: Maybe<SystemEdge>) => (
...
const AddSystemForm: React.FC<AddSystemProps> = ({ onSuccess, onCancel }) => {
  const notifDispatch = useNotificationDispatch();
  const { register, handleSubmit } = useForm<SystemInput>();
  const [save, { error }] = useMutation<any, MutationSaveSystemArgs>(
    SAVE_SYSTEM,
    {
      onCompleted(data) {
        onSuccess();
  ...
  const onSubmit = (data: SystemInput) => {
    ...
    >
      <div className="w-1/2">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <p className="text-2xl font-extrabold tracking-wider">Add System</p>
          <div className="mt-4">
            <label
              htmlFor="title"
...
const UpdateSystemForm: React.FC<UpdateSystemProps> = ({
  values,
  onUpdateSuccess,
  onDeleteSuccess,
  onCancel,
  ...
  const notifDispatch = useNotificationDispatch();
  const { register, handleSubmit } = useForm<SystemUpdateInput>({
    defaultValues: {
      title: values?.title,
    },
  });

  const [save, { error }] = useMutation<any, MutationUpdateSystemArgs>(
    UPDATE_SYSTEM,
    {
      onCompleted(data) {
        onUpdateSuccess();
  ...
  const onUpdateSubmit = (data: SystemUpdateInput) => {
